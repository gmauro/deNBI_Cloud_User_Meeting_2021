resource "openstack_compute_instance_v2" "nfs" {
  ...
  block_device {
    uuid                  = data.openstack_images_image_v2.tf_workshop-image.id
    source_type           = "image"
  ...
  block_device {
    uuid                  = openstack_blockstorage_volume_v2.tf_workshop-volume_nfs_data.id
    source_type           = "volume"
...
resource "openstack_blockstorage_volume_v2" "tf_workshop-volume_nfs_data" {
  name = "${random_id.unique_name_suffix.hex}-volume_nfs_data"
